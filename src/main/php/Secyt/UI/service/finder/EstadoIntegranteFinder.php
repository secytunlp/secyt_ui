...
/**
 * 
 * Finder para EstadoIntegrante.
 * 
 * @author Marcos
 * @since 14/04/2020
 */
class EstadoIntegranteFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIEstadoIntegranteCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIEstadoIntegranteService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIEstadoIntegranteService()->get( $code );
	}
