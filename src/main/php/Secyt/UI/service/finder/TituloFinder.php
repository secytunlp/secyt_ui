 ...
 * @since 09/04/2020
 */
class TituloFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UITituloCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUITituloService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUITituloService()->get( $code );
	}
	...
	function getEntityLabel( $entity ){
		if( !empty( $entity)  )
		return $entity->getTituloUniversidad();
	}
