...
/**
 * 
 * Finder para EstadoProyecto.
 * 
 * @author Marcos
 * @since 11/04/2020
 */
class EstadoProyectoFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIEstadoProyectoCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIEstadoProyectoService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIEstadoProyectoService()->get( $code );
	}
