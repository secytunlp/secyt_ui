...
/**
 * 
 * Finder para Disciplina.
 * 
 * @author Marcos
 * @since 11/04/2020
 */
class DisciplinaFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIDisciplinaCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIDisciplinaService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIDisciplinaService()->get( $code );
	}
