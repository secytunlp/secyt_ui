...
/**
 * 
 * Finder para TipoInvestigador.
 * 
 * @author Marcos
 * @since 10/04/2020
 */
class TipoInvestigadorFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UITipoInvestigadorCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUITipoInvestigadorService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUITipoInvestigadorService()->get( $code );
	}
