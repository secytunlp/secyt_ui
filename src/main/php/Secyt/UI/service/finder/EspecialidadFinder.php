...
/**
 * 
 * Finder para Especialidad.
 * 
 * @author Marcos
 * @since 11/04/2020
 */
class EspecialidadFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIEspecialidadCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIEspecialidadService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIEspecialidadService()->get( $code );
	}
