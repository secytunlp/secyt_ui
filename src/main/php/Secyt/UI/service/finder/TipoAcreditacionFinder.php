...
/**
 * 
 * Finder para TipoAcreditacion.
 * 
 * @author Marcos
 * @since 10/04/2020
 */
class TipoAcreditacionFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UITipoAcreditacionCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUITipoAcreditacionService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUITipoAcreditacionService()->get( $code );
	}
