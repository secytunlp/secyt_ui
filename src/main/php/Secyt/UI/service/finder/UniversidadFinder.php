...
/**
 * 
 * Finder para Universidad.
 * 
 * @author Marcos
 * @since 09/04/2020
 */
class UniversidadFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIUniversidadCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIUniversidadService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIUniversidadService()->get( $code );
	}
