...
/**
 * 
 * Finder para Categoria.
 * 
 * @author Marcos
 * @since 09/04/2020
 */
class CategoriaFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UICategoriaCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUICategoriaService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUICategoriaService()->get( $code );
	}
